...
def main():
    ...
    if hasattr(env.unwrapped, "scene"):
        ...
        if hasattr(scene, "articulations"):
            for name, art in scene.articulations.items():
                if "robot" in name.lower() or "g1" in name.lower():
                    robot = art
                    print(f"[Env] ✓ Robot found: {name}")
                    break

    # Load policy
    log_root = os.path.join(os.getcwd(), "logs", "rsl_rl", "g1_flat")
    run_dir = os.path.join(log_root, args_cli.load_run)
    policy = load_policy(run_dir, args_cli.checkpoint, env_cfg["device"])
    ...
    debug_vis = DebugVisualizer(env_cfg["num_envs"], env_cfg["device"])
    ...
    # Simulation loop
    actions = torch.zeros(env_cfg["num_envs"], env.action_space.shape[1], device=env_cfg["device"])
    sim_time = 0.0
    step_count = 0
    dt = 0.02
        ...
        while simulation_app.is_running():
            ...
            if robot is not None and arm_ik.initialized:
                ...
                default_arm_pos = torch.tensor(
                    ...
                    device=env_cfg["device"]
                ).unsqueeze(0).expand(env_cfg["num_envs"], -1)
